package net.maku.auth.provider;
...
import net.maku.auth.cache.CaffeineCache;
...
public class JwtProvider {
     ...
     * 根据用户信息生成token
     */
    public AccessToken createToken(UserDetails userDetails, Long tokenType) {
        return createToken(userDetails.getUsername(),tokenType);
    }
     ...
     * 参数是我们想放入token中的字符串
     */
    public AccessToken createToken(String subject, Long expiredTime) {
        // 当前时间
        final Date now = new Date();
        // 过期时间
        final Date expirationDate = new Date(now.getTime() + expiredTime * 1000);
     ...
     */
    public List<AccessToken> refreshToken(String oldToken) {
        String token = oldToken.substring(jwtProperties.getTokenPrefix().length());

        String username = (String) redisUtils.get(RedisKeys.getTokenKey(oldToken));

        if (StringUtils.isEmpty(username)) {
            log.warn("refreshToken已经过期");
            return null;
        }
        ...
        // 生成accessToken
        AccessToken _token = createToken(claims.getSubject(), jwtProperties.getExpirationTime());

        //如果token在30分钟之内刚刷新过，返回原refreshToken和新的accessToken
        if (tokenRefreshJustBefore(claims)) {
            AccessToken refreshToken = AccessToken.builder().loginAccount(claims.getSubject()).token(oldToken).expirationTime(claims.getExpiration()).build();
            return Arrays.asList(_token, refreshToken);
        } else {
            // 删除原缓存中的refreshToken
            redisUtils.delete(RedisKeys.getTokenKey(oldToken));
            AccessToken refreshToken = createToken(claims.getSubject(), jwtProperties.getExpirationRefreshTime());
            return Arrays.asList(_token, refreshToken);
        }
    }
